...
public class Controller {
    ...
    // Modifica 16/05/2024
    // Questo è un servizio che gestisce le operazioni relative a Google OAuth2,
    // potrebbe non servire, è solo per testare
    @Autowired
    ...
    // String regex = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d).{3,14}$"; // maiuscola,
    // minuscola e numero

    // REGEX -- Modifica (03/02/2024) : La psw deve contenere da 8 a 16 caratteri,
    // di cui almeno un carattere minuscolo,
    // maiuscolo, un numero ed un carattere speciale
    String regex = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[\\W_]).{8,16}$"; // maiuscola, minuscola, numero e chr.
                                                                           // speciale
    ...
    public ResponseEntity<String> register(@RequestParam("name") String name,
            ...
            @RequestParam("studies") Studies studies, @CookieValue(name = "jwt", required = false) String jwt,
        ...
        // MODIFICA (23/2/2024) : Commento alla riga riguardante il reCAPTCHA perchè non
        // più utilizzato
        // verifyReCAPTCHA(gRecaptchaResponse);

        User n = new User();

        // NOME -- Modifica (02/02/2024) : Possibilità di inserire più nomi separati da
        // uno spazio
        ...
        // COGNOME --Modifica (02/02/2024) : Possibilità di inserire più parole separate
        // da uno spazio ed eventualmente
        // da un apostrofo
        // regex_old = "[a-zA-Z]+" , regex_new = [a-zA-Z]+(\s?[a-zA-Z]+\'?)*
        // if ((name.length() >= 2) && (surname.length() <= 30) &&
        // (Pattern.matches("[a-zA-Z]+", surname))) {
        if ((name.length() >= 2) && (surname.length() <= 30)
                && (Pattern.matches("[a-zA-Z]+(\\s?[a-zA-Z]+\\'?)*", surname))) {
        ...
        if ((password.length() > 16) || (password.length() < 8) || !(m.matches())) {
            ...
            // return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Password not
            // valid, maiuscola, minuscola e numero, con lunghezza tra 8 e 16");
    ...
    // restTemplate.postForEntity(recaptchaServerURL, request, String.class);
    ...
    public ResponseEntity<String> login(@RequestParam("email") String email,
            @RequestParam("password") String password, @CookieValue(name = "jwt", required = false) String jwt,
        ...
        if (user == null) {
            ...
            return ResponseEntity.status(HttpStatus.UNAUTHORIZED)
                    .body("User already registered with facebook, please log in with facebook");
        }
    ...
    public ResponseEntity<String> login_with_facebook(@RequestParam("email") String email,
            ...
            @RequestParam("access_token") String tokenFb, @CookieValue(name = "jwt", required = false) String jwt,
            HttpServletRequest request, HttpServletResponse response) {
        ...
        // Esegue la richiesta GET e ottiene la risposta come oggetto
        ...
        if (is_valid == true) {
            ...
            if (user != null) {
                ...
                if (user.isRegisteredWithFacebook) {
                    ...
                    return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
                            "Ti sei già registrato con email e password. Nella pagina di login, inserisci le tue credenziali.");
                }
            } else {
                ...
                try {
                    ...
                    try {
                        ...
                        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                                .body("Failed to confirm your registration");
                    }
                    // FINE MODIFICA
                } catch (MessagingException e) {
                    return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
    ...
    public ResponseEntity<String> logout(@RequestParam("authToken") String authToken, HttpServletResponse response,
        ...
        // Modifica 18/05/2024: Cancellazione dei cookie e del contesto di
        // autenticazione di spring
    ...
    // MODIFICA (03/02/2024) : Feedback registrazione avvenuta con successo +
    ...
    // Questo metodo reindirizza l'utente alla pagina di autorizzazione di Google
    // per il login
