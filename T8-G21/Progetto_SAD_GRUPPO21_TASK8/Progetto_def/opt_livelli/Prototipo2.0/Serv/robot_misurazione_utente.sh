...
if [[ $PERCORSO_TEST =~ .*\/Allenamento\/.* ]]; then
    # Esegui il comando solo se è modalità allenamento
    cp -f evosuite-report/statistics.csv "$(dirname $PERCORSO_TEST)/TestData.csv"
else
