...
import com.groom.manvsclass.repository.*;
import com.groom.manvsclass.responses.FileUploadResponse;
...
import com.groom.manvsclass.util.FileDownloadUtil;
import com.groom.manvsclass.util.FileUploadUtil;
import com.groom.manvsclass.util.RobotUtil;
...
public class AdminService {
    ...
    public ModelAndView showHomeAdmin(HttpServletRequest request, String jwt) {
        if (jwtService.isJwtValid(jwt))
            return new ModelAndView("home_adm");
    ...
    public ResponseEntity<ClassUT> uploadClasse(ClassUT classe, String jwt) {
        if (jwtService.isJwtValid(jwt)) {
            ...
            Operation operation1 = new Operation((int) orepo.count(), userAdmin.getUsername(), classe.getName(), 0,
                    data);
    ...
    public ResponseEntity<FileUploadResponse> uploadFile(MultipartFile classFile, String model, String jwt,
            HttpServletRequest request) throws IOException {
        if (jwtService.isJwtValid(jwt)) {
            // Legge i metadati della classe della parte "model" del body HTTP e li salva in
            // un oggetto ClasseUT
    ...
    public ResponseEntity<FileUploadResponse> uploadTest(MultipartFile classFile, String model, MultipartFile testFile,
            MultipartFile testFileEvo, String jwt, HttpServletRequest request) throws IOException {
        if (jwtService.isJwtValid(jwt)) {
            ...
            if (testFile != null)
                fileNameTest = StringUtils.cleanPath(testFile.getOriginalFilename());
            else
                fileNameTest = null;

            String fileNameTestEvo;

            if (testFileEvo != null)
                fileNameTestEvo = StringUtils.cleanPath(testFileEvo.getOriginalFilename());
            else
                fileNameTestEvo = null;

            RobotUtil.saveRobots(fileNameClass, fileNameTest, fileNameTestEvo, classe.getName(), classFile, testFile,
                    testFileEvo);
            ...
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
            String data = currentDate.format(formatter);
            Operation operation1 = new Operation((int) orepo.count(), userAdmin.getUsername(),
                    classe.getName() + " con Robot", 0, data);
        ...
        if (jwtService.isJwtValid(jwt)) {
            ...
            if (folderToDelete.exists() && folderToDelete.isDirectory()) {
                try {
                    ...
                    if (directoryRandoop.exists() && directoryRandoop.isDirectory())
                        FileUploadUtil.deleteDirectory(directoryRandoop);

                    if (directoryEvo.exists() && directoryEvo.isDirectory())
                        FileUploadUtil.deleteDirectory(directoryEvo);

                    return new ResponseEntity<>("Cartella eliminata con successo (/deleteFile/{fileName})",
                            HttpStatus.OK);
                } catch (IOException e) {
    ...
    public ResponseEntity<String> modificaClasse(String name, ClassUT newContent, String jwt,
        ...
        if (jwtService.isJwtValid(jwt)) {
            ...
            if (modifiedCount > 0) {
                ...
                DateTimeFormatter formatter = DateTimeFormatter.ofPattern("yyyy-MM-dd");
                String data = currentDate.format(formatter);
                Operation operation1 = new Operation((int) orepo.count(), userAdmin.getUsername(), newContent.getName(),
                        1, data);
                orepo.save(operation1);
                return new ResponseEntity<>("Aggiornamento eseguito correttamente.", HttpStatus.OK);
            } else {
                return new ResponseEntity<>("Nessuna classe trovata o nessuna modifica effettuata.",
    ...
    public ResponseEntity<?> registraAdmin(Admin admin1, String jwt) {
        ...
        if (admin1.getNome().length() >= 2 && admin1.getNome().length() <= 30
                && Pattern.matches("[a-zA-Z]+(\\s[a-zA-Z]+)*", admin1.getNome())) {
            this.userAdmin.setNome(admin1.getNome());
        } else {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Nome non valido");
        }

        // 2: Possibilità di inserire più parole separate da uno spazio ed eventualmente
        // da un apostrofo
        if (admin1.getCognome().length() >= 2 && admin1.getCognome().length() <= 30
                && Pattern.matches("[a-zA-Z]+(\\s?[a-zA-Z]+\\'?)*", admin1.getCognome())) {
            this.userAdmin.setCognome(admin1.getCognome());
        } else {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Cognome non valido");
        }

        // 3: L'username deve rispettare necessariamente il seguente formato: "[username
        // di lunghezza compresa tra 2 e 30 caratteri]_unina"
        if (admin1.getUsername().length() >= 2 && admin1.getUsername().length() <= 30
                && Pattern.matches(".*_unina$", admin1.getUsername())) {
            this.userAdmin.setUsername(admin1.getUsername());
        } else {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
                    "Username non valido, deve rispettare il seguente formato: [username di lunghezza compresa tra 2 e 30 caratteri]_unina");
        }

        // 4: L'email deve essere necessariamente quella istituzionale e terminare: o
        // con [nome]@studenti.unina.it oppure [nome]@unina.it
        if (Pattern.matches("^[a-zA-Z0-9._%+-]+@(?:studenti\\.)?unina\\.it$", admin1.getEmail())) {
            ...
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("Email non valida, registrarsi con le credenziali istituzionali!");
        }

        // 5: La password deve contenere almeno una lettera maiuscola, una minuscola, un
        // numero ed un carattere speciale e deve essere lunga tra gli 8 e i 16
        // caratteri
        Matcher m = Pattern.compile("^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{8,16}$")
                .matcher(admin1.getPassword());
        if (admin1.getPassword().length() > 16 || admin1.getPassword().length() < 8 || !m.matches()) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
                    "Password non valida! La password deve contenere almeno una lettera maiuscola, una minuscola, un numero ed un carattere speciale e deve essere lunga tra gli 8 e i 16 caratteri");
        }
    ...
    public ModelAndView showChangePswAdminForm(HttpServletRequest request, String jwt) {
        if (jwtService.isJwtValid(jwt))
            return new ModelAndView("redirect:/login_admin");
    ...
    public ResponseEntity<?> resetPasswordAdmin(Admin admin1, String jwt) {
        ...
        try {
            ...
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Errore nell'invio del messaggio di posta");
        }
    }
    ...
    public ResponseEntity<?> inviteAdmins(Admin admin1, String jwt) {
        ...
        // Controlliamo che non esista nel repository un admin con la mail specificata
        // nell'invito
        Admin admin = arepo.findById(admin1.getEmail()).orElse(null);
        if (admin != null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("Email trovata, la persona che stai tentando di invitare è già un amministratore!");
        }
        ...
        try {
            ...
            return ResponseEntity.ok()
                    .body("Invitation token inviato correttamente all'indirizzo:" + savedAdmin.getEmail());
        } catch (MessagingException e) {
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR)
                    .body("Errore nell'invio del messaggio di posta");
        }
    ...
    public ResponseEntity<?> loginWithInvitation(Admin admin1, String jwt) {
        ...
        if (admin1.getUsername().length() >= 2 && admin1.getUsername().length() <= 30
                && Pattern.matches(".*_invited$", admin1.getUsername())) {
            admin.setUsername(admin1.getUsername());
        } else {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(
                    "Username non valido, deve rispettare il seguente formato: [username di lunghezza compresa tra 2 e 30 caratteri]_invited");
        }
    ...
    public ResponseEntity<List<String>> getRobots(HttpServletRequest request, String jwt) {
        ...
        try {
            // Leggi il file e crea una lista di stringhe
            List<String> robots = Files.readAllLines(Paths.get(configRobotPath));
            return ResponseEntity.ok(robots); // Risponde con una lista di robot
        } catch (IOException e) {
            return ResponseEntity.status(500).body(null); // Gestisce errori di lettura
        }
    }
}