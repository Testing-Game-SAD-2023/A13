...
type Service interface {
	Create(request *CreateRequest) (ScalataGame, error)
	FindById(id int64) (ScalataGame, error)
	Delete(id int64) error
	Update(id int64, ug *UpdateRequest) (ScalataGame, error)
	FindByInterval(accountId string, i api.IntervalParams, p api.PaginationParams) ([]ScalataGame, int64, error)
}
...
func (gc *Controller) List(w http.ResponseWriter, r *http.Request) error {
	...
	scalataGames, count, err := gc.service.FindByInterval(accountId.AsString(), ip, pp)
	if err != nil {
		return api.MakeHttpError(err)
	}

	return api.WriteJson(w, http.StatusOK, api.MakePaginatedResponse(scalataGames, count, pp))
}
