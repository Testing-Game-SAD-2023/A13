...
In **Scalata mode**, each level corresponds to a single Round. When a player fails a level, the same Round is reused (incrementing an internal attempt counter), while successful completion triggers the creation of a new Round for the next level. This reuse of the Round entity allows leveraging the existing architecture without structural modifications.
...
In **Scalata mode**, each attempt at a level generates one or more Turns associated with the current Round, maintaining consistency with the existing turn-based gameplay model.
