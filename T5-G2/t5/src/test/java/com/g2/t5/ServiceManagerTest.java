...
public class ServiceManagerTest {
	...
	public void testRegisterService_ValidService() {
		for (int i = 0; i < serviceNames.size(); i++) {
			...
			assertNotNull(serviceManager.services.get(serviceName),
					"Il servizio non è stato registrato correttamente.");
		}
	}
	...
	public void testHandleRequest_ErrorInServiceExecution() {
		for (int i = 0; i < serviceNames.size(); i++) {
			String serviceName = serviceNames.get(i);
			System.out
					.println("Inizio test di gestione richiesta con errore di esecuzione per servizio: " + serviceName);

			// Pre-condizioni: Mock di un servizio, registrazione del servizio e simulazione
			// di errore
			ServiceInterface mockService = mock(ServiceInterface.class);
			when(mockService.handleRequest("error", new Object[] {}))
					.thenThrow(new RuntimeException("Errore simulato"));
			serviceManager.services.put(serviceName, mockService);
			RestTemplate mockRestTemplate = createMockRestTemplate(HttpStatus.INTERNAL_SERVER_ERROR, "Errore simulato");

			// Azione: Gestione della richiesta "error"
			Object result = serviceManager.handleRequest(serviceName, "error");

			// Post-condizioni: Verifica che il risultato sia null
			assertNull(result,
					"Il risultato non dovrebbe essere null in caso di errore per il servizio: " + serviceName);
		}
	}
	...
	// Test T7: createService con costruttore non valido
	@Test
	public void testCreateService_InvalidConstructor() {
		// Classe di servizio senza costruttore valido
		class InvalidService implements ServiceInterface {
			@Override
			public Object handleRequest(String action, Object... params) {
		...
		// Azione: Tentativo di creare InvalidService
		Exception exception = assertThrows(RuntimeException.class, () -> {
			serviceManager.createService(InvalidService.class, mockRestTemplate);
		});
