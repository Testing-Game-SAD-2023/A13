...
public class AchievementService {
    ...
    public List<AchievementProgress> updateProgressByPlayer(int playerID) {
        ...
        List<AchievementProgress> achievementProgressesPrevious = getProgressesByPlayer(playerID).stream()
                .filter(a -> a.Progress >= a.ProgressRequired).toList();
        ...
        for (Statistic statistic : statisticList) {
            ...
            if (statistic.getGamemode() != Gamemode.All)
                filteredGameList = gamesList.stream()
                        .filter(game -> Objects.equals(game.getDescription(), statistic.getGamemode().toString()))
                        .toList();

            // TODO: filtrare anche per Robot (allo stato attuale non vi è associazione in
            // DB tra robot e partita)
        ...
        List<AchievementProgress> obtainedAchievements = new ArrayList<>(
                getProgressesByPlayer(playerID).stream().filter(a -> a.Progress >= a.ProgressRequired).toList());
    ...
    private void setProgress(int playerID, String statisticID, float progress) {
        // restTemplate.put("http://t4-g18-app-1:3000/phca/" + playerID + "/" +
        // statisticID, new StatisticProgress(playerID, statisticID, progress));
    ...
    public List<AchievementProgress> getProgressesByPlayer(int playerID) {
        ...
        for (Achievement a : achievementList) {
            List<StatisticProgress> filteredList = categoryProgressList.stream()
                    .filter(cat -> Objects.equals(cat.getStatisticID(), a.getStatisticID())).toList();
            for (StatisticProgress c : filteredList)
                achievementProgresses.add(new AchievementProgress(a.getID(), a.getName(), a.getDescription(),
                        a.getProgressRequired(), c.getProgress()));
            if (filteredList.size() == 0) // if there is no progress recorded, just put progress 0
                achievementProgresses.add(new AchievementProgress(a.getID(), a.getName(), a.getDescription(),
                        a.getProgressRequired(), 0));
        }
    ...
    public List<StatisticProgress> getStatisticsByPlayer(int playerID) {
        List<StatisticProgress> statisticProgresses = (List<StatisticProgress>) serviceManager.handleRequest("T4",
                "getStatisticsProgresses", playerID);
        ...
        statisticProgresses
                .removeIf(x -> !statisticList.stream().anyMatch(y -> Objects.equals(y.getID(), x.getStatisticID())));

        for (Statistic statistic : statisticList) {
            // se non c'è il progresso salvato in db, aggiungilo manualmente impostandolo a
            // 0
            if (!statisticProgresses.stream()
                    .anyMatch(progress -> Objects.equals(progress.getStatisticID(), statistic.getID())))
