...
$(document).ready(function () {
    ...
    startGame(data);
        ...
        if(data.mode === "Allenamento"){
            run_button.disabled =true; // Abilita/disabilita in base alla modalità
            coverage_button.disabled = false; // Abilita il pulsante di coverage
            console.log("Sono in Allenamento");
        }else if(data.mode === "Scalata"){
            run_button.disabled =false; // Abilita/disabilita in base alla modalità
            coverage_button.disabled = true; // Abilita/disabilita il pulsante di coverage
            console.log("Sono in Scalata");
        }else if(data.mode === "Sfida"){
            run_button.disabled =false; // Abilita/disabilita in base alla modalità
            coverage_button.disabled = false; // Abilita/disabilita il pulsante di coverage
            console.log("Sono in Sfida");
        }
...
async function processCoverage(coverage, formData, robotScore, userScore, isGameEnd, loadingKey, buttonKey, coverageDetails) {
    ...
    const mode = localStorage.getItem("modalita"); 
        ...
        if(mode=="Scalata"){
            if (robotScore>userScore) {handleGameLoss();
            } else {handleGameProgress(userScore);}
        }else{
            if (isGameEnd) {handleEndGame(userScore); // Gestisce la fine del gioco
            } else{resetButtons();}  
        }
}
